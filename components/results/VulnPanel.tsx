  ...
  if (vulnerabilities.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-900 border rounded-xl p-5 shadow-sm">
        <h2 className="text-sm font-semibold mb-4 text-gray-900 dark:text-gray-100">Vulnerability scan</h2>
        <p className="text-gray-500 dark:text-gray-400">No vulnerability data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border rounded-xl p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Vulnerability scan</h2>
      </div>
            ...
            >
              <div className="flex items-center gap-2">
                {vuln.safe ? (
                  ...
                  <X className="h-4 w-4 text-red-700 dark:text-red-400 flex-shrink-0" />
                )}
                <span className="text-sm text-gray-800 dark:text-gray-200">{vuln.name}</span>
              </div>
              {learn && (
                <LearnTooltip
                  title={learn.title}
